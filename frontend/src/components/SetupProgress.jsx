...
const SetupProgress = ({ onStatus }) => {
  const [counts, setCounts] = useState({ departments:0, buildings:0, courses:0, offerings:0, professors:0, students:0 });
  ...
  useEffect(()=>{
    const load = async ()=>{
      try{
        ...
        const [deps, bds, cs, ofs, profs, users] = await Promise.all([
          fetch('/api/departments/'), fetch('/api/buildings/'), fetch('/api/courses/'), fetch('/api/course-offerings/'), fetch('/api/professors/'), fetch('/api/admin/users/', { headers })
        ]);
        const res = {};
  ...
  // Progress counts exclude timeslots (generated by algorithm). 4 core items + professors
  const totalItems = 5;
  const have = (counts.departments>0?1:0) + (counts.buildings>0?1:0) + (counts.courses>0?1:0) + (counts.offerings>0?1:0) + (counts.professors>0?1:0);
