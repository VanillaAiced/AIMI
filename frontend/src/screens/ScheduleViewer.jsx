...
const ScheduleViewer = ()=>{
  ...
  const getUniqueBlocks = () => {
    ...
    entries.forEach(e => {
      if(e.block) {
        ...
        const name = block.code || block.name || `Block ${key}`;
        if(!seen.has(key)) {
  ...
  const getUniqueProfs = () => {
    ...
    entries.forEach(e => {
      if(e.professor) {
        const prof = typeof e.professor === 'object' ? e.professor : {id: e.professor, name: `Prof ${e.professor}`};
        if(!seen.has(prof.id)) {
          seen.set(prof.id, {id: prof.id, name: prof.name});
        }
      }
    });
    return Array.from(seen.values()).sort((a,b)=>String(a.name).localeCompare(String(b.name)));
  ...
  const getUniqueRooms = () => {
    ...
    entries.forEach(e => {
      if(e.room) {
        const room = typeof e.room === 'object' ? e.room : {id: e.room, name: `Room ${e.room}`};
        if(!seen.has(room.id)) {
          seen.set(room.id, {id: room.id, name: room.name});
        }
      }
    });
    return Array.from(seen.values()).sort((a,b)=>String(a.name).localeCompare(String(b.name)));
  ...
  const filteredEntries = selectedId ? entries.filter(e => {
    ...
    if(view === 'prof') {
      const profId = typeof e.professor === 'object' ? e.professor.id : e.professor;
      return String(profId) === String(selectedId);
    }
    if(view === 'room') {
      const roomId = typeof e.room === 'object' ? e.room.id : e.room;
      return String(roomId) === String(selectedId);
    }
    return false;
  }) : [];
