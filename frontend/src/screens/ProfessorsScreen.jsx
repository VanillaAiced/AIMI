...
const ProfessorsScreen = ()=>{
  ...
  const add=async(e)=>{ e.preventDefault(); const resp=await fetch('/api/professors/',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name})}); if(resp.ok){ setList(l=>[...l,await resp.json()]); setName(''); } };
