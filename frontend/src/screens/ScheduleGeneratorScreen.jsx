...
const ScheduleGeneratorScreen = ()=>{
  ...
  const onStatus = (counts) => {
    // require at least one in each category
    const ok = counts.courses>0 && counts.timeslots>0 && counts.buildings>0 && counts.offerings>0;
    setReady(ok);
  };
  const run = async ()=>{
    setRunning(true);
  ...
  return (
    <div>
      ...
      <div className="mt-3">
        ...
        {!ready && <div className="text-muted small mt-2">Generate disabled — please ensure Courses, TimeSlots, Buildings and Course Offerings exist.</div>}
      </div>
    </div>
  );
};
