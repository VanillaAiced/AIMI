...
const CourseOfferingsScreen = ()=>{
  ...
  const add=async(e)=>{ e.preventDefault(); const resp=await fetch('/api/course-offerings/',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({course:{code:course,name:course},assigned_block:{code:block}})}); if(resp.ok){ setList(l=>[...l,await resp.json()]); setCourse(''); setBlock(''); } };
